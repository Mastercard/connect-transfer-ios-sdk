...
public class ConnectViewController: UIViewController {
    ...
    internal var messageNameConnect = "maOBiOSConnect"
    ...
    public func load(_ connectUrl: String, redirectUrl:String? = nil) {
    ...
    func loadChildWebView(url: URL) {
        self.getUrlRequestWithErrorHandling(url: url) { success in
            if(!success){
                self.isChildWebViewLoaded = true
                self.childWebView = SFSafariViewController(url: url)
                self.childWebView.delegate = self
                self.childWebView.modalPresentationStyle = .overFullScreen
                self.present(self.childWebView, animated: true, completion: nil)
            }
        }
    }
    
    func getUrlRequestWithErrorHandling(url: URL,completion: @escaping completionHandler){
        OpenLinkURL.inAnyNativeWay(url: url) { success in
            if !success {
                completion(false)
            }
        }
    ...
    @objc func pingConnect() {
        let javascript = "window.postMessage({ type: 'ping', sdkVersion: '\(sdkVersion())', platform: 'iOS'" + ((self.redirectUrl != nil && self.redirectUrl != "") ? ", redirectUrl:'\(self.redirectUrl!)'": "" ) + "}"  + ", '\(self.connectUrl)')"
        self.webView.evaluateJavaScript(javascript)
    }
